...
export const WALL_PHOTO_ORDER = 8
...
export const isWallPhoto = (type: string) => type === 'photo' || type.startsWith('photo-frame')
export const wallItemOrder = (type: string) => isWallPhoto(type) ? WALL_PHOTO_ORDER : isWallMedia(type) ? WALL_MEDIA_ORDER : WALL_DECOR_ORDER

if (import.meta.env.DEV) console.assert(wallItemOrder('photo-frame-1') < WALL_VIDEO_ORDER && WALL_VIDEO_ORDER < wallItemOrder('poster') && wallItemOrder('poster') < wallItemOrder('clock') && wallItemOrder('clock') < ROOM_OBJECT_ORDER, 'room layer order must be photo < video < poster < wall decor < floor furniture')
