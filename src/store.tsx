...
import { customObjectTemplate, forgetJob, generatedModelBlob, inspectCustomModel, loadCustomObjects, pendingJob, pollGlbObject, recoverLostObjects, rememberJob, saveCustomObjects, submitGlbObject, type GeneratedModel } from './services/customObjects'
...
type RoomStore = {
  ...
  customObjects: CustomObjectSpec[]; recoverCustomObjects: () => void; addCustomObject: (spec: CustomObjectSpec) => void; renameCustomObject: (id: string, name: string) => void; removeCustomObject: (id: string) => void
  customEditing: CustomObjectSpec | null; startCustomObjectEdit: (id: string) => void; updateCustomObjectEdit: (patch: Partial<Pick<CustomObjectSpec, 'name' | 'footprint' | 'modelScale' | 'pose'>>) => void; applyCustomObjectEdit: () => void; cancelCustomObjectEdit: () => void
  customJob: CustomJob | null; runCustomGeneration: (input: { category: CustomObjectCategory; prompt: string; image?: string; size?: { width: number; depth: number; height?: number }; finish?: 'gloss'; pose?: CustomPose }) => void; markCustomSeen: () => void
...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  // 버킷에는 남아 있는데 목록에서 사라진 모델 되살리기 — 커스텀 탭을 열 때 세션에 한 번만 훑는다
  const recovering = useRef(false)
  const recoverCustomObjects = () => {
    if (recovering.current || isVisiting()) return
    recovering.current = true
    void recoverLostObjects(customObjects).then((found) => {
      if (!found.length) return
      setCustomObjects((current) => {
        const next = [...current, ...found.filter((spec) => !current.some((value) => value.id === spec.id))]
        saveCustomObjects(next)
        return next
      })
    }).catch(() => { /* 복구 실패는 조용히 넘어간다 */ })
  }
  ...
  const selectObject = (object: Exclude<SelectedObject, null>) => { const target = furniture.find((value) => value.id === object); const type = target?.type ?? object; if (mode === 'edit') { if (object !== 'character' && object !== 'book') setSelectedFurnitureId(object); return } if (type === 'diary-book') { const bookId = object.startsWith('inventory-book-') ? object.slice('inventory-book-'.length) : ''; const book = books.find((value) => value.id === bookId); if (!book || (isVisiting() && book.visibility !== 'public')) return; closePanels(); setFloorTarget(null); openBook(bookId); return } if (type.startsWith('video-frame') && videoLinks[object]) { if (!playingFrames.includes(object)) { setFrameMuted(object, false); setPlayingFrames((prev) => [...prev, object]) } return }
    ...
    // Mobile may dispatch a second tap after the first has already started a walk. Repeating the same target must
    // not cancel that walk (beds used to clear the selection), and tapping the moving character must not turn
    // walking -> idle -> sittingFloor in place.
    if ((characterState === 'walking' || characterState === 'aligning') && (type === 'character' || (selectedObject === object && isPosedItem(target)))) return
  ...
  const updateCustomObjectEdit = (patch: Partial<Pick<CustomObjectSpec, 'name' | 'footprint' | 'modelScale' | 'pose'>>) => {
...
export function NeighbourRoomProvider({ bundle, handle, children }: { bundle: Record<string, string> | null; handle?: string; children: ReactNode }) {
  const value = useMemo<RoomStore>(() => readingBundle(bundle ?? {}, () => {
    ...
    return {
      ...
      customObjects: loadCustomObjects(), recoverCustomObjects: noop, addCustomObject: noop, renameCustomObject: noop, removeCustomObject: noop, customEditing: null, startCustomObjectEdit: noop, updateCustomObjectEdit: noop, applyCustomObjectEdit: noop, cancelCustomObjectEdit: noop, customJob: null, runCustomGeneration: noop, markCustomSeen: noop,
