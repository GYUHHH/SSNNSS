...
export function RoomProvider({ children }: { children: ReactNode }) {
  // read once — it is the pose to come back in, not a live value
  const initialPose = useRef(loadInteractions().pose).current
...
export function NeighbourRoomProvider({ bundle, children }: { bundle: Record<string, string> | null; children: ReactNode }) {
  const value = useMemo<RoomStore>(() => readingBundle(bundle ?? {}, () => {
    ...
    return {
      ...
      readOnly: true, characterHome: storedCharacterHome(), characterPose: interactions.pose,
