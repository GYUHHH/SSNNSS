...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  // a linked frame starts playing on entry, muted — browsers block sound before any user gesture
  const [playingFrame, setPlayingFrame] = useState<string | null>(() => furniture.find((item) => !item.removed && item.type.startsWith('video-frame') && videoLinks[item.id])?.id ?? null)
  const [wallMuted, setWallMuted] = useState(playingFrame !== null)
