...
export const inventoryItems: Array<Omit<FurnitureItem, 'id' | 'position' | 'surfaceId' | 'gridX' | 'gridY' | 'gridZ' | 'wallId' | 'rotation' | 'updatedAt'>> = [
  ...
  { type: 'glass-table', name: '유리 테이블', category: 'floorFurniture', movable: true, interactable: true, footprint: { width: 2, depth: 1 }, size: [2, 1], scale: 1, allowedSurfaces: ['floor'] },
  ...
  { type: 'pop-shelf', name: '팝 선반', category: 'floorFurniture', movable: true, interactable: true, footprint: { width: 2, depth: 1 }, size: [2, 1], scale: 1, allowedSurfaces: ['floor'] },
  { type: 'bubble-chair', name: '버블 체어', category: 'floorFurniture', movable: true, interactable: true, footprint: { width: 2, depth: 2 }, size: [2, 2], scale: 1, allowedSurfaces: ['floor'] },
  { type: 'y2k-desk', name: 'Y2K 책상', category: 'floorFurniture', movable: true, interactable: true, footprint: { width: 2, depth: 1 }, size: [2, 1], scale: 1, allowedSurfaces: ['floor'] },
  { type: 'pod-daybed', name: '포드 데이베드', category: 'floorFurniture', movable: true, interactable: true, footprint: { width: 3, depth: 3 }, size: [3, 3], scale: 1, allowedSurfaces: ['floor'] },
  ...
  { type: 'star-dust', name: '별가루', category: 'decoration', movable: true, interactable: true, footprint: { width: 2, depth: 2 }, size: [2, 2], scale: 1, allowedSurfaces: ['floor'] },
  { type: 'speech-bubble', name: '말풍선', category: 'decoration', movable: true, interactable: true, footprint: { width: 1, depth: 1 }, size: [1, 1], scale: 1, allowedSurfaces: ['floor'] },
...
type RoomStore = {
  ...
  openStyleTarget: (target: StyleTarget) => void; setWallStyle: (wallId: WallId, presetId: string) => void; setFloorStyle: (presetId: string) => void; setFloorImage: (image: string | null) => void; setFurnitureStyle: (id: FurnitureId, presetId: string) => void
}
...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  const setFloorImage = (image: string | null) => {
    if (isVisiting()) return
    const roomId = activeRoomId
    const save = (value: string | undefined) => saveSlotStyle(roomId, { ...(slotStyle(roomId) ?? {}), floorImage: value })
    const next = image ?? undefined
    save(next); setFloorImageState(next); setWallStyleState((current) => ({ ...current, floorImage: next }))
    if (image?.startsWith('data:')) void uploadDataUrl('floor', image).then((url) => {
      if (!url) return
      save(url)
      if (activeRoomIdRef.current !== roomId) return
      setFloorImageState((current) => current === image ? url : current)
      setWallStyleState((current) => current.floorImage === image ? { ...current, floorImage: url } : current)
    })
  }
...
export function NeighbourRoomProvider({ bundle, handle, children }: { bundle: Record<string, string> | null; handle?: string; children: ReactNode }) {
  const value = useMemo<RoomStore>(() => readingBundle(bundle ?? {}, () => {
    ...
    return {
      ...
      customObjects: loadCustomObjects(), addCustomObject: noop, removeCustomObject: noop, customJob: null, runCustomGeneration: noop, markCustomSeen: noop,
      ...
      openStyleTarget: noop, setWallStyle: noop, setFloorStyle: noop, setFloorImage: noop, setFurnitureStyle: noop, toggleDebugAnchors: noop,
    }
  }), [bundle, handle])
