...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadRemoteLikes = () => void fetchAllLikes().then((rows) => {
      ...
      rows.forEach((row) => { if (row.visitor !== mine) counts[row.item_id] = (counts[row.item_id] ?? 0) + 1 })
