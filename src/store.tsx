...
type RoomStore = {
  ...
  customJob: CustomJob | null; resumeCustomGeneration: (requestId: string) => void; runCustomGeneration: (input: { category: CustomObjectCategory; prompt: string; image?: string; size?: { width: number; depth: number; height?: number }; finish?: 'gloss'; pose?: CustomPose }) => void; markCustomSeen: () => void
...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  // fal에는 결과가 남아 있는데 앱이 요청 번호를 잃은 경우(BUILD 770 이전 사고) 번호만 알면 되살릴 수 있다
  const resumeCustomGeneration = (requestId: string) => {
    const id = requestId.trim()
    if (!id || (customJob && customJob.stage !== 'done' && customJob.stage !== 'error')) return
    setCustomJob({ stage: 'draft', round: 1, unseen: false })
    void awaitGeneration(id, { category: 'furniture', prompt: '' })
  }
