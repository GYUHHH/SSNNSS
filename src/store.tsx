...
export const objectInfo: Record<string, { title: string; subtitle: string }> = { bookshelf: { title: '책장', subtitle: '개인 기록' }, computer: { title: '컴퓨터', subtitle: '작업 공간' }, desk: { title: '책상', subtitle: '오늘 할 일' }, chair: { title: '의자', subtitle: '책상 의자' }, sofa: { title: '소파', subtitle: '잠깐 쉬어가기' }, bed: { title: '침대', subtitle: '편안한 휴식' }, character: { title: '나', subtitle: '창가에서 쉬는 중' }, plant: { title: '화분', subtitle: '물을 기다리는 중' }, lamp: { title: '스탠드 조명', subtitle: '따뜻한 불빛' }, clock: { title: '벽 시계', subtitle: '현재 시간' }, poster: { title: '포스터', subtitle: '좋아하는 색' }, photo: { title: '사진', subtitle: '여름의 바다' }, cup: { title: '머그컵', subtitle: '따뜻한 커피' }, cabinet: { title: '수납장', subtitle: '작은 물건들' }, bin: { title: '휴지통', subtitle: '책상 옆' }, rug: { title: '러그', subtitle: '포근한 발밑' } }
...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  const clearSelection = () => { setSelectedObject(null); setFloorTarget(null); setCupHeld(false); setBookshelfOpen(false); setOpenBookId(null); setSelectedFurnitureId(null) }
  const selectObject = (object: Exclude<SelectedObject, null>) => { const target = furniture.find((value) => value.id === object); const type = target?.type ?? object; if (mode === 'edit') { if (object !== 'character' && object !== 'book') setSelectedFurnitureId(object); return } if (type === 'character') { setCharacterState((state) => ({ idle: 'sittingFloor', sittingFloor: 'wave', wave: 'idle' } as Partial<Record<CharacterState, CharacterState>>)[state] ?? 'idle'); return } if (type === 'bed' && selectedObject === object) return clearSelection(); setFloorTarget(null); setSelectedObject(object); setOpenBookId(null); if (type === 'bookshelf') setBookshelfOpen(true); const sidePanelOnly = type === 'bookshelf' || type === 'guestbook' || type === 'photo' || type === 'poster' || type.startsWith('photo-frame') || type.startsWith('wall-art'); if (sidePanelOnly) return; if (type === 'computer') setComputerOn((on) => !on); if (['lamp', 'floor-lamp', 'fireplace', 'candle', 'tv', 'string-lights', 'christmas-tree', 'star-projector', 'mini-fridge', 'led-lamp'].includes(type)) setLitLamps((prev) => { const next = new Set(prev); if (next.has(object)) next.delete(object); else next.add(object); return next }); if (type === 'cup') setCupHeld(true); if (!['lamp', 'floor-lamp', 'fireplace', 'candle', 'tv', 'computer', 'music-player', 'string-lights', 'christmas-tree', 'star-projector', 'mini-fridge', 'record-player', 'whiteboard', 'led-lamp'].includes(type)) setCharacterState(target?.category === 'wallItem' ? 'interacting' : 'walking') }
  ...
  useEffect(() => {
    ...
    fetch('/instagram/import.json').then((response) => response.ok ? response.json() : null).then((book: Book | null) => {
      if (!book) return
