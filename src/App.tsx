...
function Interface() {
  ...
  return <main className={artOpen ? 'app art-open' : 'app'}>
    ...
    {mode === 'normal' && <><div className="room-controls"><nav className="zoom-controls" aria-label="화면 확대축소"><button type="button" onClick={() => zoom(-4)} aria-label="축소">−</button><button type="button" onClick={() => zoom(4)} aria-label="확대">+</button></nav><button className="inventory-button" type="button" onClick={() => { setInventoryOpen(true); toggleEditMode() }}>가구함</button></div><nav className="room-slots" aria-label="내 방 목록">{rooms.map((room) => <button key={room.id} type="button" className={room.id === activeRoomId ? 'active' : ''} onClick={() => room.id !== activeRoomId && openRoom(room.id)} onDoubleClick={() => rooms.length > 1 && setConfirmingRoom(room.id)}>{room.name}</button>)}<button type="button" className="add-room" onClick={createRoom} aria-label="새 방 만들기">+</button></nav><nav className="time-controls" aria-label="시간대">{([['day', '낮'], ['evening', '저녁'], ['night', '밤']] as const).map(([key, label]) => <button key={key} type="button" className={timeOfDay === key ? 'active' : ''} onClick={() => setTimeOfDay(key)}>{label}</button>)}</nav></>}
