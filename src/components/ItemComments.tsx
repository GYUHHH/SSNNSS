...
export default function ItemComments() {
  ...
  return <div className="overlay" onMouseDown={(event) => event.currentTarget === event.target && setCommentTarget(null)}>
    <section className="reaction-card item-comments comment-ui" aria-label={t('댓글')}>
      <div className="comment-head">
        <strong>{item?.name ?? ''}</strong>
        {/* 댓글 달린 영상액자는 클릭이 이 팝업으로 먼저 가므로, 수정 패널은 여기서 연다 (주인만) */}
        {!isVisiting() && item?.type.startsWith('video-frame') && <button type="button" className="edit-frame" aria-label={t('영상 수정')} onClick={() => { setCommentTarget(null); openVideoPanel(commentTarget) }}>✎</button>}
      </div>
      ...
      <div className="guest-list">
        {comments.map((comment) => <article key={comment.id} className="guest-note comment-item">
          ...
          <header><CommentName name={comment.name} /><time>{timeAgo(comment.createdAt)}</time>{(!isVisiting() || comment.visitor === mine) && <button type="button" aria-label={t('삭제')} onClick={() => removeGuestComment(commentTarget, comment.id)}>×</button>}</header>
          <p>{comment.text}</p>
        </article>)}
      </div>
    </section>
