...
import { embedSrc, trackIframe, resumedPlaylistVideo, watchPlaylistOrder, playFrame, framePlayerStates } from '../services/ytResume'
import { clipIsPlaying, loadClipUrls, playClip } from '../services/mediaStore'
import { playbackKey } from '../services/playbackSession'
...
export default function WallVideoLayer() {
  ...
  return <>{playingFrames.map((id) => <WallVideo key={`${currentHandle ?? 'lobby'}:${activeRoomId}:${id}`} frameId={id} sessionKey={playbackKey(currentHandle, activeRoomId, id, 'youtube')} />)}</>
}

function WallVideo({ frameId, sessionKey }: { frameId: string; sessionKey: string }) {
  ...
  useEffect(() => {
    ...
    let live = true
    const lookupId = videoId.startsWith('pl:') ? (resumedPlaylistVideo(sessionKey) ?? videoId.split('@')[1]) : videoId
    if (!lookupId) return
    ...
    return () => { live = false }
  }, [active, videoId, frameId, sessionKey, divHeight])
    ...
    <group rotation={[0, 0, -item.rotation[1]]}>
      ...
      <Html transform occlude="blending" geometry={<planeGeometry args={[screenWidth, screenHeight]} />} distanceFactor={400} position={[0, 0, .042]} scale={screenWidth / 640} zIndexRange={[4, 0]} style={{ pointerEvents: mode === 'edit' ? 'none' : 'auto' }}>
        <div className="wall-video" style={{ width: 640, height: divHeight, pointerEvents: mode === 'edit' ? 'none' : 'auto' }}>
          ...
          <ResumingIframe key={frameId} videoId={videoId} frameId={frameId} sessionKey={sessionKey} extra="autoplay=1&playsinline=1&mute=1&controls=0" frameStyle={{ ...(crop ? { width: 640, top: -crop, height: divHeight + crop * 2 } : { width: 640, height: divHeight }), pointerEvents: mode === 'edit' ? 'none' : 'auto' }} />
...
export function ResumingIframe({ videoId, frameId, sessionKey, extra, frameStyle }: { videoId: string; frameId: string; sessionKey: string; extra: string; frameStyle?: React.CSSProperties }) {
  const frame = useRef<HTMLIFrameElement>(null)
  const [src] = useState(() => embedSrc(videoId, frameId, sessionKey, extra))
  useEffect(() => { if (frame.current) return trackIframe(frame.current, frameId, sessionKey) }, [frameId, sessionKey])
