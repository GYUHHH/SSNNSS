  ...
  return <FollowFit fitName={`fit:${item.id}`} synced={fitSynced}>
    <group rotation={[0, 0, -item.rotation[1]]}>
      <group ref={screen} position={[0, 0, .042]}>
        ...
        <div ref={element} className="wall-video" data-frame-id={frameId} data-video-meta={display === undefined ? 'loading' : display ? 'ready' : 'failed'} data-video-aspect={display?.aspect ?? ''} style={{ width: 640, height: divHeight, pointerEvents: mode === 'edit' ? 'none' : 'auto' }}>
