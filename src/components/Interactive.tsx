...
export default function Interactive({ id, position, rotation = [0, 0, 0], scale: baseScale = 1, children }: Props) {
  ...
  const { selectObject, enterEditFurniture, furniture, guestbook, pendingReactions, markReactionsSeen, setReactionTarget, setCommentTarget } = useRoomStore()
  ...
  const hasComments = (guestbook[id] ?? []).some((comment) => comment.visitor && comment.visitor !== myVisitorId())
  useCursor(hovered)
