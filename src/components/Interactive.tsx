...
export default function Interactive({ id, position, rotation = [0, 0, 0], scale: baseScale = 1, pad: padded = true, editing = false, editOverlay, children }: Props) {
  ...
  return <group ref={group} position={position} rotation={rotation} scale={baseScale} userData={{ interactive: id }}
    ...
    onClick={(event) => { if (readOnly || padOverruled(event)) return; event.stopPropagation(); if (editing) return; if (longPressed.current) { longPressed.current = false; return }; if (!directPanel && openObject(id)) return; if (!directPanel && !isPosedItem(item)) { openReactionPicker({ id, x: event.clientX, y: event.clientY }); return }; selectObject(id) }}>
