...
export default function CustomSizeEditor() {
  ...
  return <section className="custom-size-editor" aria-label={t('모델 크기 수정')} onPointerDown={(event) => event.stopPropagation()}>
    ...
    {customEditing.category === 'furniture' && <label className="custom-pose"><span>{t('캐릭터 동작')}</span><select value={customEditing.pose ?? ''} onChange={(event) => updateCustomObjectEdit({ pose: (event.target.value || undefined) as CustomPose | undefined })}><option value="">{t('자동')}</option><option value="sit">{t('앉기')}</option><option value="lie">{t('눕기')}</option></select></label>}
