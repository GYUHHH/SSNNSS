...
import { clampModelScale, type CustomObjectSpec, type CustomPose } from '../customObjectSpec'
...
export default function CustomSizeEditor() {
  ...
  const reset = () => {
    ...
    updateCustomObjectEdit({ name: value.name, footprint: value.footprint, modelScale: value.modelScale ?? [1, 1, 1], pose: value.pose })
  }
  return <section className="custom-size-editor" aria-label={t('모델 크기 수정')} onPointerDown={(event) => event.stopPropagation()}>
    ...
    {customEditing.category === 'furniture' && <label className="custom-pose"><span>{t('캐릭터 동작')}</span><select value={customEditing.pose ?? ''} onChange={(event) => updateCustomObjectEdit({ pose: (event.target.value || undefined) as CustomPose | undefined })}><option value="">{t('없음')}</option><option value="sit">{t('앉기')}</option><option value="lie">{t('눕기')}</option></select></label>}
