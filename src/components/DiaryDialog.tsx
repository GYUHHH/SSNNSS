...
export default function DiaryDialog() {
  ...
  return <>
    <section className="diary" aria-label={book.title}>
      <header className="diary-head"><div className="diary-title"><button className="diary-back" type="button" aria-label={writing ? '기록 목록으로' : '책장으로'} onClick={() => writing ? setWriting(false) : closeBook()}>←</button></div>{!isVisiting() && !writing && <div className="diary-head-actions"><button type="button" onClick={() => setWriting(true)}>새 기록 작성</button></div>}</header>
...
function EntryComments({ entry, inputRef }: { entry: Entry; inputRef: React.RefObject<HTMLTextAreaElement | null> }) {
  ...
  return <section className="entry-comments comment-ui" aria-label="댓글">
    ...
    <div className="entry-comment-list">{comments.map((comment) => <article key={comment.id} className="entry-comment comment-item"><CommentAvatar name={comment.name} photo={comment.photo} /><header><CommentName name={comment.name} /><time>{timeAgo(comment.createdAt)}</time>{(!isVisiting() || comment.visitor === mine) && <button type="button" aria-label="댓글 삭제" onClick={() => removeGuestComment(entry.id, comment.id)}>×</button>}</header><p>{comment.text}</p></article>)}</div>
  </section>
}
