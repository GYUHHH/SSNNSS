...
export function ItemVisual({ item, preview = false }: { item: FurnitureItem; preview?: boolean }) {
  ...
  if (item.type === 'mirror') return <group>
...
function BannerArt({ id }: { id: string }) {
  ...
  return <mesh position={[0, 0, .075]}><planeGeometry args={[1.88, .44]} /><meshBasicMaterial map={texture} /></mesh>
}
