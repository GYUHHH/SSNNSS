...
export default function StylePanel() {
  const { styleTarget, closeStyleTarget, furniture, wallStyle, floorStyle, setWallStyle, setFloorStyle, setFurnitureStyle } = useRoomStore()
  if (!styleTarget) return null

  if (styleTarget.kind === 'floor') {
    const current = floorStyleOf(floorStyle).id
    return <section className="style-panel" aria-label="바닥 꾸미기"><header><strong>바닥 스타일</strong><button className="close-ui" type="button" aria-label="닫기" onClick={closeStyleTarget}>×</button></header>
  ...
  return <section className="style-panel" aria-label={`${title} 꾸미기`}><header><strong>{title} 색상</strong><button className="close-ui" type="button" aria-label="닫기" onClick={closeStyleTarget}>×</button></header>
