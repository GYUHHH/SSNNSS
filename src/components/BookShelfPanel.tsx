...
export default function BookShelfPanel() {
  ...
  return <>
    <section className="bookshelf-panel" aria-label={t('책장')}>
      {!isVisiting() && adding && <form className="new-book" onSubmit={submit}>
        <input aria-label={t('새 책 제목')} value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t('새 책 제목')} />
        <button type="submit">{t('책 만들기')}</button>
      </form>}
      <div className="book-list">{books.map((book) => {
        const draft = draftOf(book)
        return <div key={book.id} className="book-row-wrap">
          {managing
            ? <div className="book-row"><i style={{ background: book.coverColor }} /><input className="book-title-input" aria-label={tp('{title} 제목', { title: book.title })} maxLength={50} value={draft.title} onChange={(event) => edit(book.id, { title: event.target.value })} /></div>
          ...
          {!isVisiting() && managing && <div className="book-controls"><button type="button" onClick={() => edit(book.id, { visibility: draft.visibility === 'public' ? 'private' : 'public' })}>{draft.visibility === 'public' ? t('공개') : t('비공개')}</button><button type="button" aria-label={tp('{title} 단 설정', { title: book.title })} onClick={() => edit(book.id, { shelf: (draft.shelf + 1) % tiers })}>{tp('{n}단', { n: draft.shelf + 1 })}</button><button className="book-delete" type="button" onClick={() => setDeleting(book.id)}>{t('삭제')}</button></div>}
          {!isVisiting() && deleting === book.id && <div className="delete-confirm"><span>{tp('‘{title}’ 삭제할까요?', { title: book.title })}</span><button type="button" onClick={() => setDeleting(null)}>{t('취소')}</button><button type="button" onClick={() => { deleteBook(book.id); setDeleting(null) }}>{t('삭제')}</button></div>}
        </div>
      })}</div>
      {!isVisiting() && <div className="bookshelf-tools"><button className="book-create" type="button" aria-label={t('새 책 만들기')} onClick={() => { setAdding(true); cancelManaging() }}>+</button><button className="book-manage" type="button" aria-label={managing ? t('책 관리 닫기') : t('책 관리')} onClick={() => managing ? cancelManaging() : beginManaging()}>{managing ? '×' : <PencilIcon />}</button>{managing && changed.length > 0 && <button className="book-save" type="button" disabled={!valid} onClick={saveChanges}>{t('완료')}</button>}</div>}
    </section>
  </>
}
