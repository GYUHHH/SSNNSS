...
export default function BookShelfPanel() {
  ...
  return <>
    <section className="bookshelf-panel" aria-label="책장">
      ...
      <h2>책장</h2>
      <div className="book-list">{books.map((book) => <div key={book.id} className="book-row-wrap"><button className="book-row" type="button" onClick={() => openBook(book.id)}><i style={{ background: book.coverColor }} /><span>{book.title}</span><small>{book.visibility === 'public' ? '공개' : '비공개'}</small></button><select className="book-shelf-pick" aria-label={`${book.title} 단 선택`} value={Math.min(book.shelf ?? 0, tiers - 1)} onChange={(event) => setBookShelf(book.id, Number(event.target.value))}>{Array.from({ length: tiers }, (_, index) => <option key={index} value={index}>{index + 1}단</option>)}</select><button className="book-delete" type="button" onClick={() => setDeleting(book.id)}>삭제</button>{deleting === book.id && <div className="delete-confirm"><span>‘{book.title}’ 삭제할까요?</span><button type="button" onClick={() => setDeleting(null)}>취소</button><button type="button" onClick={() => { deleteBook(book.id); setDeleting(null) }}>삭제</button></div>}</div>)}</div>
