...
function CrossfadingLights({ preset }: { preset: typeof LIGHTING[keyof typeof LIGHTING] }) {
  ...
  useFrame(({ camera, size }, delta) => {
    ...
    if (dir.current) {
      ...
      dir.current.shadow.intensity = MathUtils.damp(dir.current.shadow.intensity, zoomedIn ? .42 : 0, 6, step)
  ...
  return <>
    ...
    <directionalLight ref={dir} castShadow position={[4, 8, -2]} intensity={initial.dir} color={initial.dirColor} shadow-mapSize-width={512} shadow-mapSize-height={512} shadow-camera-left={-8} shadow-camera-right={8} shadow-camera-top={8} shadow-camera-bottom={-8} />
  </>
}
