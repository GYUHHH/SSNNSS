...
type InventoryEntry = { type: string; name: string; size: [number, number]; footprint: FurnitureItem['footprint']; allowedSurfaces: FurnitureItem['allowedSurfaces']; styleId?: string; restoreId?: string }
...
export default function InventoryPanel() {
  ...
  const { startPreview, preview, previewValid, placePreview, availableCount, furniture } = useRoomStore()
  ...
  const legacyStored: InventoryEntry[] = furniture.filter((item) => item.removed && LEGACY_FRAME_TYPES.has(item.type)).map((item) => ({ type: item.type, name: item.name, size: [item.footprint.width, item.footprint.depth], footprint: item.footprint, allowedSurfaces: item.allowedSurfaces, styleId: item.styleId, restoreId: item.id }))
  const stock = showingColors || showingCharacter || showingBooks ? [] : [...CATALOG.filter((entry) => availableCount(entry.type) > 0), ...legacyStored].filter((entry) => tab === '전체' || (entry.type === 'speech-bubble' ? '소품' : categoryFor(entry.type)) === tab as InventoryCategory)
  return <section className={preview ? 'inventory-panel previewing' : 'inventory-panel'} aria-label={t('보관함')}>
    ...
    {showingBooks ? <BooksTab /> : showingCharacter ? <CharacterLookEditor /> : showingColors ? <RoomColorEditor /> : <div className="inventory-items">
      ...
      {stock.map((entry) => <button key={entry.restoreId ?? `${entry.type}:${entry.styleId ?? ''}`} type="button" onClick={() => startPreview(entry.type, entry.styleId, entry.restoreId)}><ItemIcon item={entry as FurnitureItem} /><span>{t(entry.name)}<small>{entry.footprint.width ? `${entry.size[0]} × ${entry.size[1]}` : t('벽')}</small></span></button>)}
    </div>}
