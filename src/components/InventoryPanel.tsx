...
function CustomTab() {
  ...
  return <div ref={root} className="custom-tab">
    ...
    {source && <form className="custom-form" onSubmit={submit}>
      ...
      {category === 'furniture' && <label>{t('캐릭터 동작')}<select value={pose} onChange={(event) => setPose(event.target.value as '' | CustomPose)}><option value="">{t('자동')}</option><option value="sit">{t('앉기')}</option><option value="lie">{t('눕기')}</option></select></label>}
